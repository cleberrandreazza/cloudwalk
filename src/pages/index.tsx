...
export default function Home({ planets, characters }: HomeProps) {
  const [itens, setItens] = useState<FilterItem[]>(
  ...
  const filterPlanets = itens
    .filter((x) => x.checked)
    .map((planet) => planet.homeWorld)
  const filteredCharacters = characters.results.filter((person) =>
    filterPlanets.includes(person.homeworld)
  )
  ...
  return (
    <>
      ...
      <Characters
        data={filterPlanets.length ? filteredCharacters : characters.results}
      />
    </>
  )
}
...
const getAllPlanets = async () => {
  ...
  while (hasMorePages) {
    ...
    newPlanets = {
      ...planets,
      results: newPlanets.results
        ? newPlanets.results.concat(planets.results)
        : planets.results,
    }
    page++
  }
...
const getAllPeople = async () => {
  ...
  while (hasMorePages) {
    ...
    newPeople = {
      ...people,
      results: newPeople.results
        ? newPeople.results.concat(people.results)
        : people.results,
    }
    page++
  }
...
export const getStaticProps: GetStaticProps<HomeProps> = async () => {
  ...
  people.results.map((x) => {
    x.homeworld_string = planets.results.filter(
      (i) => i.url === x.homeworld
    )[0].name
  })
